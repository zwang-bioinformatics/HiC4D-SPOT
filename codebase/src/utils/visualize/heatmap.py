...
"""
def drawRotatedHalfHeatmapUp(fig,ax, C, vmax):
	n = C.shape[0]
	...
	# cmap = 'coolwarm' #'sns.blend_palette(['white', 'red'], as_cmap=True)
...
def drawNonmalHeatmap(fig,ax, C, vmax):
	n = C.shape[0]
	ax.set_xlim([0,n])
	ax.set_ylim([0,n])
	ax.xaxis.set_ticks(np.arange(0, n, n/4))
...
def drawRotatedHalfHeatmapUp2(fig,ax, C, vmin, vmax, cmap):
	n = C.shape[0]
	A = rotateCountMatrixUp(n)
	ax.set_xlim([0,n])
	ax.set_ylim([0,n])
...
def drawVectorHeatmap2(ax,vector,colormap,vmin,vmax):
	n = vector.shape[0]
	vec = vector.reshape((1,n))
	ax.set_xlim((0,n))
	ax.tick_params(which='both',direction='out',
		...
		labelbottom=False,labelleft=False,
		length=4)
	sns.heatmap(vec,vmin=vmin,vmax = vmax,cmap=colormap,ax=ax,cbar=False)
	ax.xaxis.set_ticks(np.arange(0, n, n/4))
